...
parser.add_argument("--dino_checkpoint", type=str, default='dinov3_vitb16_pretrain_lvd1689m-73cec8be.pth')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if os.path.isfile(args.load_ckpt):
        ...
        lora_keys = [k for k in checkpoint['state_dict'].keys() if 'lora' in k]
        if len(lora_keys) > 0:
            print(f"✅ Loaded {len(lora_keys)} LoRA parameters from checkpoint.")
        else:
            print("⚠️ WARNING: No LoRA parameters found in checkpoint!")
            
        del checkpoint
    else:
        print("=> no checkpoint found at '{}'!!!!!!!".format(args.load_ckpt))
        raise FileNotFoundError(f"Checkpoint not found at {args.load_ckpt}")

    from utils.eval_utils import eval_relight_ViT
    for i in range(12):
        error, ssim = eval_relight_ViT(args, 100, model)
        print("⏰🍋Round {}: error {}, ssim {}".format(i+1, error, ssim))
    exit()
