...
def get_surface_norma_dataloader(args):
...
def get_eval_relight_dataloader(args, eval_pair_folder_shift = 5, eval_pair_light_shift = 5, img_size = 256):
...
def eval_relight(args, epoch, model, eval_pair_folder_shift = 5, eval_pair_light_shift = 5):
    ...
    train_loader = get_eval_relight_dataloader(args, eval_pair_folder_shift, eval_pair_light_shift)
    t0 = time.time()
    epoch_iter = len(train_loader)
    P_mean=-0.5
    P_std=1.2
...
@torch.no_grad()
def eval_relight_ViT(args, epoch, model, eval_pair_folder_shift = 5, eval_pair_light_shift = 5):
    '''
    model: ViT model
    '''
    torch.manual_seed(args.gpu)
    train_loader = get_eval_relight_dataloader(args, eval_pair_folder_shift, eval_pair_light_shift, img_size = 224)
    t0 = time.time()
    epoch_iter = len(train_loader)
    P_mean=-0.5
    P_std=1.2
