...
class MeanFlow:
    ...
    def loss(self, model, x_intri, x_extri, c=None):
        batch_size = x_intri.shape[0]
        device = x_intri.device
        ...
        e = torch.randn_like(x_intri)
        x_intri = self.normer.norm(x_intri)
        x_extri = self.normer.norm(x_extri)
    ...
    @torch.no_grad()
    def evaluate(self, model, x_intri, max_t = 0.8, c=None,
        ...
        print(z.shape, x_intri.shape)
        z = (1 - max_t) * x_intri + max_t * z

        t_vals = torch.linspace(max_t, 0.0, sample_steps + 1, device=device)
