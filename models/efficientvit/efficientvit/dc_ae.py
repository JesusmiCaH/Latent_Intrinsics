...
__all__ = ["DCAE", "dc_ae_f32c32", "dc_ae_f64c128", "dc_ae_f128c512", "dc_ae_f16c768_v2", "dc_ae_f16c1024_v2"]
...
def dc_ae_f16c768_v2(name: str, pretrained_path: Optional[str] = None) -> DCAEConfig:
    # 4 upsamples (16x), input 768
    # We only use the decoder part mostly, but defining full config for consistency
    # 5 stages. 4 upsamples means we go from 1/16 -> 1/8 -> 1/4 -> 1/2 -> 1/1
    # width_list should be compatible with 768 input?
    # DINOv3 embedding is 768.
    # User said "define a new dc-ae prototype with only 5 stages (4 upsamples)"
    # Typical DC-AE configs have symmetric encoder/decoder.
    # Let's define a reasonable width list starting at 128 and going up to 768?
    # Or simplified.
    if name == "dc-ae-f16c768-v2":
        cfg_str = (
            "latent_channels=768 "
            "encoder.block_type=[ResBlock,ResBlock,EViT_GLU,EViT_GLU,EViT_GLU] "
            "encoder.width_list=[128,256,512,768,768] encoder.depth_list=[2,2,2,2,2] "
            "decoder.block_type=[ResBlock,ResBlock,EViT_GLU,EViT_GLU,EViT_GLU] "
            "decoder.width_list=[128,256,512,768,768] decoder.depth_list=[2,2,2,2,2] "
            "decoder.norm=[bn2d,bn2d,trms2d,trms2d,trms2d] decoder.act=[relu,relu,silu,silu,silu]"
        )
    else:
        raise NotImplementedError
    cfg = OmegaConf.from_dotlist(cfg_str.split(" "))
...
def dc_ae_f16c1024_v2(name: str, pretrained_path: Optional[str] = None) -> DCAEConfig:
    if name == "dc-ae-f16c1024-v2":
        cfg_str = (
            "latent_channels=1024 "
            "encoder.block_type=[ResBlock,ResBlock,EViT_GLU,EViT_GLU,EViT_GLU] "
            "encoder.width_list=[128,256,512,1024,1024] encoder.depth_list=[2,2,2,2,2] "
            "decoder.block_type=[ResBlock,ResBlock,EViT_GLU,EViT_GLU,EViT_GLU] "
            "decoder.width_list=[128,256,512,1024,1024] decoder.depth_list=[2,2,2,2,2] "
            "decoder.norm=[bn2d,bn2d,trms2d,trms2d,trms2d] decoder.act=[relu,relu,silu,silu,silu]"
        )
    else:
        raise NotImplementedError
