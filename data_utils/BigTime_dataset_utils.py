...
class BigTime_Dataset_PreLoad(data.Dataset):
    def __init__(self, root, img_transform, epoch_multiplier = 10,
        ...
        self.images = parallel_load_image(img_list, data_filter = '/00/*')
        self.epoch_multiplier = epoch_multiplier
        folder_lens = [len(imgs) for imgs in self.images]
        self.cum_lens = np.cumsum([0] + folder_lens)
        self.single_img_transform = img_transform[1]
        self.group_img_transform = img_transform[0]

    def __len__(self):
        return self.epoch_multiplier * self.cum_lens[-1]

    def __getitem__(self, index):
        index = index % (self.__len__() // self.epoch_multiplier)
        folder_idx = np.searchsorted(self.cum_lens, index, side="right") - 1
        images = self.images[folder_idx]

        img_num = len(images)
        light_index = np.random.randint(img_num)

        img1 = Image.fromarray(images[light_index])
        img2 = Image.fromarray(images[(light_index + 1 + np.random.randint(img_num - 1)) % img_num])
